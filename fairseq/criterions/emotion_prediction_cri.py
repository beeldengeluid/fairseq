...
class EmotionPredictionCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        #targets = model.get_targets(sample, [logits]).view(-1) #MODIFIED
        ...
        if self.args.regression_target_mos:
            ...
            #targets = targets.long() MODIFIED

            # loss = F.nll_loss( #MODIFIED
            #     F.log_softmax(logits, dim=-1, dtype=torch.float32),
            #     targets,
            #     reduction='sum',
            # )
            ...
            preds = logits.max(dim=1)[1]
            #ncorrect=(preds == targets).sum().item() MODIFIED
            probabilities = F.softmax(logits, dim=-1)
            probabilities_s = tuple(probabilities[0].squeeze().tolist())
            logging_output = {preds[0].item(),probabilities_s }
