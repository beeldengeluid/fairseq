...
class RawAudioTextVideoDataset(FairseqDataset):
    ...
    def collater(self, samples):
        ...
        #print(len(sources))
        #print(sources)
        sizes = [len(s) for s in sources] #MODIFIED sizes = [len(s['cpc_logits']) for s in sources] #MODIFIED
        ...
        collated_sources = sources[0].new(len(sources), target_size) #Is this for make the sizes alike #MODIFIED  collated_sources = sources[0]['cpc_logits'].new(len(sources), target_size) 
        padded_amount=[]
        for i, (source, size) in enumerate(zip(sources, sizes)): #MODIFIED
            ...
            #assert diff >= 0
            if diff == 0:
                collated_sources[i] = source#['cpc_logits'] (MODIFIED)
            elif diff > 0:
                start = np.random.randint(0, diff + 1)
                end = size - diff + start
                collated_sources[i] = source[start:end] #MODIFIED collated_sources[i] = source['cpc_logits'][start:end] 
            else:
                delta = abs(diff)
                #m = torch.nn.ConstantPad1d((0,delta),source[-1]) #left padding with the last element
                m = torch.nn.ConstantPad1d((0,delta),0)
                collated_sources[i] = m(source) #MODIFIED collated_sources[i] = m(source['cpc_logits'])
